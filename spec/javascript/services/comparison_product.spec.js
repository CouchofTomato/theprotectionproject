...
describe("ComparisonProduct", () => {
  const product = new ComparisonProduct(
    data.insurer,
    data.product,
  ...
  describe("overallSumAssured", () => {
    it("returns the sum assured of the core module selected", () => {
      expect(product.overallSumAssured()).toEqual(
        "USD 500,000 | GBP 300,000 | EUR 400,000"
      );
    });
  });
  ...
  describe("productModuleBenefit", () => {
    ...
    it("adds the benefit icon html as a property", () => {
      expect(product.productModuleBenefit(2)).toHaveProperty(
        "benefit_icon",
