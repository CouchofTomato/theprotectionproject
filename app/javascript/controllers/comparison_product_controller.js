...
export default class extends Controller {
  ...
  addSelectedProductToComparison(data) {
    this.comparisonProduct = new ComparisonProduct(
      data.insurer,
      data.product,
      data.product_modules,
      data.module_benefits
    );
  ...
  selectedProductModules() {
    const productModules = Array.from(
      this.productModulesTarget.querySelectorAll("input[type=radio]:checked")
    );
  ...
  addBenefits() {
    ...
    this.benefitRowTargets.forEach((benefitRow) => {
      const productModuleBenefit =
        this.comparisonProduct.productModuleBenefit(benefitRow.id) ||
        nullProductModuleBenefit;
